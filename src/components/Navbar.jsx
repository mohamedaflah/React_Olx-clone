      ...
      <div className="w-[90%] md:w-[95%] lg:w-[83%]  flex items-center  justify-between">
        ...
        {user && (
          <div className="flex items-center justify-start p-0 cursor-pointer text-1xl">
            <FaUserAlt /> <div onClick={()=>setOpenProfile(!profile)} style={{rotate:profile?"180deg":"0deg",transition:"0.4s ease"}}><Arrow  /></div>
            {profile && <div className="absolute   w-40 p-2 top-16 bg-[#494747d1] h-36  z-10 items-start justify-start" style={{transition:".4s ease"}}>
              <div className="border-b">{user}</div>
              <div>
                <button className="border-b border-black cursor-pointer" onClick={()=>{
                  signOut(auth).then(()=>{
                    setUser(null)
                  })
                }}>Logout</button>
